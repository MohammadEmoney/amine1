...
class LiveUserEdit extends Component
{
    ...
    public function mount()
    {
        ...
        $this->dropout = $this->user->dropout ? true : false;
    ...
    public function loadData()
    {
        ...
        if($this->user->dropout){
            $this->dropouts = [
                'description' => $this->user->dropout->description,
                'reasons' => $this->user->dropout->reasons,
                'date_left' => $this->user->dropout->date_left ? Jalalian::fromDateTime($this->user->dropout->date_left)->format('Y-m-d') : null,
                'date_return' => $this->user->dropout->date_return ? Jalalian::fromDateTime($this->user->dropout->date_return)->format('Y-m-d') : null,
            ];
        }
    }
    ...
    public function studentValidation()
    {
        $this->validate([
            ...
            'data.refferal_national_code' => ['nullable', 'string', 'max:255'],
    ...
    public function submitStudent()
    {
        try {
            ...
            if($this->dropout){
                $user->dropout()->updateOrCreate([
                    'user_id' => Auth::id(),
                    'description' => $this->dropouts['description'] ?? null,
                    'reasons' => $this->dropouts['reasons'] ?? null,
                    'date_left' => $this->dropouts['date_left'] ?? null,
                    'date_return' => $this->dropouts['date_return'] ?? null,
                ]);
            }else{
                $user->dropout()->forceDelete();
            }
            DB::commit();
    ...
    public function render()
    {
        ...
        return view('livewire.admin.users.live-user-edit', compact('initialLevels', 'courses', 'educations', 'roles', 'permissions', 'militaryStatuses', 'dropoutReasons'));
    }
}
