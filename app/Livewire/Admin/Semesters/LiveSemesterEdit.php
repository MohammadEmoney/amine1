...
class LiveSemesterEdit extends Component
{
    ...
    public function mount(Semester $semester)
    {
        ...
        $this->data['time_start'] =  $this->semester->time_start?->format("H:i");
        $this->data['time_end'] =  $this->semester->time_end?->format("H:i");
        ...
        // $this->data['register_date'] = Jalalian::fromDateTime($this->semester->register_date)->format('Y-m-d');
    ...
    public function validations()
    {
        $this->validate(
            ...
            [
                ...
                'data.register_date' => 'شروع ثبت نام',
    ...
    public function render()
    {
        ...
        $staff = User::query()->whereDoesntHave('dropout')->with('roles')->get()->filter(
            fn ($user) => $user->roles->where('name', 'teacher')->toArray()
        );
        $students = User::with(['userInfo', 'dropout'])
