        ...
        <div class="container-fluid">
            ...
            <div class="collapse justify-content-center navbar-collapse" id="navbarNav">
                <ul class="bg-ac-info border d-flex justify-content-between m-0 navbar-nav p-0" dir="rtl">
                    ...
                    <li class="nav-item border-end">
                        <a class="nav-link nav-icon-hover" href="{{ route('profile.classes.index') }}" aria-expanded="false">
                            <span>
                    ...
                    @if (auth()->user()->userInfo?->type == "staff")
                        ...
                        <li class="nav-item border-end">
                            <a class="nav-link nav-icon-hover" href="{{ route('profile.evaluations.index') }}" aria-expanded="false">
                                <span>
                                    <i class="ti ti-help"></i>
                                </span>
                                <span class="hide-menu text-white">سوال ها</span>
                            </a>
                        </li>
                        <li class="nav-item border-end">
                            <a class="nav-link nav-icon-hover" href="{{ route('profile.attendances.classes') }}" aria-expanded="false">
                                <span>
                                    <i class="ti ti-user-check"></i>
                                </span>
                                <span class="hide-menu text-white">حضور غیاب</span>
                            </a>
                        </li>
                    @endif
                    @if(auth()->user()->userInfo?->type == "student")
                        <li class="nav-item">
                            <a class="nav-link nav-icon-hover border-end border-start" href="{{ route('profile.checkout.index') }}" aria-expanded="false">
                                <span>
                                    <i class="ti ti-file-description"></i>
                                </span>
                                <span class="hide-menu text-white position-relative"> درخواست های من 
                                    @if (Auth::user()->carts?->count())
                                        <span class="position-absolute top-0 translate-middle p-1 bg-danger border border-light rounded-circle" style="right: 95%">
