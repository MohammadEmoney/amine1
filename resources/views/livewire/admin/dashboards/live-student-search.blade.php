            ...
            <div class="card w-100 shadow">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between">
                        ...
                        <button class="btn btn-ac-primary" wire:click="createStudent()">ایجاد دانش آموز</button>
                    </div>
                                            ...
                                            @foreach ($users as $key => $user)
                                                <tr>
                                                    ...
                                                    @else
                                                        -
                                                    @endif </td>
                                                    <td class="text-nowrap">{{ $user->orders()->latest()->book()->first()?->trans_status ?: "-"}}</td>
                                                    <td class="text-nowrap">{{ $user->orders()->latest()->book()->first()?->contract_number ?: "-"}}</td>
                                                    <td class="text-nowrap">{{ $user->orders()->latest()->tuition()->first()?->trans_status ?: "-"}}</td>
                                                    <td class="text-nowrap">{{ $user->orders()->latest()->tuition()->first()?->contract_number ?: "-"}}</td>
                                                    <td>-</td>
                                                    <td>{{ $user->orders()->latest()->tuition()->first()?->remaining_amount ?: "-" }}</td>
                                                    <td>
                                                        <p class="text-nowrap">کتاب: <span dir="ltr">{{ $user->orders()->latest()->book()->first()?->persian_register_date ?: "-"}}</span></p>
                                                        <p class="text-nowrap">شهریه: <span dir="ltr">{{ $user->orders()->latest()->tuition()->first()?->persian_register_date ?: "-"}}</span></p>
                                                    </td>
                                                    <td>{{ $user->orders()->latest()->tuition()->first()?->paid_amount ?: "-"}}</td>
                                                    <td>{{ $user->orders()->latest()->tuition()->first()?->description ?: "-" }}</td>
    ...
    @can('semester_access')
        <div class="col-lg-12">
            <div class="d-flex justify-content-between">
                ...
                <button class="btn btn-ac-primary" wire:click="createSemester()">ایجاد کلاس</button>
            </div>
