<div class="card mt-3">
    <div class="card-body">
        <h5 class="text-center">چند گزینه ای</h5>
        @foreach ($questions as $key => $question)
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    @foreach ($questions[$key]['choices'] ?? [] as $choiceKey => $choice)
                        <div class="row">
                            <div class="col-md-2">
                                <div class="mb-3">
                                    <label for="exampleInputtext1" class="form-label">شناسه گزینه
                                        *</label>
                                    <input type="text" class="form-control"
                                        wire:model.live.debounce.800ms="questions.{{ $key }}.choices.{{ $choiceKey }}.id" id="exampleInputtext1" min="1"
                                        aria-describedby="textHelp" placeholder="">
                                    <div>
                                        @error('questions.{{ $key }}.choices.{{ $choiceKey }}.id')
                                            {{ $message }}
                                        @enderror
                                    </div>
                                </div>
                            ...
                            <div class="col-md-9">
                                <div class="mb-3">
                                    <label for="exampleInputtext1" class="form-label">گزینه
                                        *</label>
                                    <input type="text" class="form-control text-start"
                                        wire:model.live.debounce.800ms="questions.{{ $key }}.choices.{{ $choiceKey }}.title" id="exampleInputtext1" min="1"
                                        aria-describedby="textHelp" placeholder="Example: Reading Task 1">
                                    <div>
                                        @error('questions.{{ $key }}.choices.{{ $choiceKey }}.title')
                                            {{ $message }}
                                        @enderror
                                    </div>
                                </div>
                            ...
                            <div class="col-md-1 pt-3">
                                <div class="mb-3 mb-3 pt-3">
                                    <button type="button" class="btn btn-sm btn-danger" wire:click="deleteChoice({{ $key }}, {{ $choiceKey }})"><i class="ti ti-minus"></i></button>
                                </div>
                            </div>
                        </div>
                    @endforeach
                    <div class="text-center">
                        <button type="button" class="btn btn-primary" wire:click="addChoice({{ $key }})"><i class="ti ti-plus"></i> افزودن گزینه</button>
                    </div>
                    <div class="my-3 row">
